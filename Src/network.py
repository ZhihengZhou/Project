    ...
    def generator(self, x, is_training):
        ...
        with tf.variable_scope('generator'):
            ...
            with tf.variable_scope('deconv1'):
                x = deconv_layer(x, [4, 4, 128, 256], [self.batch_size, 64, 40, 128], 2)
                x = batch_normalize(x, is_training)
                x = tf.nn.relu(x)
            ...
            with tf.variable_scope('deconv2'):
                x = deconv_layer(x, [4, 4, 64, 128], [self.batch_size, 128, 80, 64], 2)
                x = batch_normalize(x, is_training)
                x = tf.nn.relu(x)
            ...
            with tf.variable_scope('deconv3'):
                x = deconv_layer(x, [4, 4, 32, 64], [self.batch_size, 256, 160, 32], 2)
                x = batch_normalize(x, is_training)
                x = tf.nn.relu(x)
